...
export function IoTAlertPopup({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      <div
        className={cn(
          "bg-white rounded-lg border shadow-xl w-full max-w-md animate-in slide-in-from-top-2 duration-200",
        )}
      >
        ...
        <div className="p-4 space-y-2">
          <div className="text-sm">
            <span className="font-medium">Máquina:</span> {alert.machine_id}
          </div>
          <div className="text-sm">
            <span className="font-medium">Métrica:</span> {alert.metric} ={" "}
            {alert.value}
          </div>
          <div className="text-sm">
            <span className="font-medium">Prioridade:</span> {alert.priority}
          </div>
          {alert.message ? (
            <div className="text-sm">
              <span className="font-medium">Mensagem:</span> {alert.message}
            </div>
          ) : null}
          <div className="text-xs text-muted-foreground flex items-center gap-1">
            <Clock className="h-3 w-3" /> {formatSince(alert.created_at)}
          </div>
        </div>
        <div className="p-4 border-t flex gap-2 justify-end">
          <button
            ...
            className="px-3 py-2 rounded bg-gray-200 text-gray-800 hover:bg-gray-300 flex items-center gap-2"
          >
...
export function IoTAlertPopupContainer() {
  ...
  const canNotify = useMemo(
    () =>
      authService.hasAnyRole(["maintenance", "admin"]) ||
      authService.hasRole("maintenance"),
    [],
  );
