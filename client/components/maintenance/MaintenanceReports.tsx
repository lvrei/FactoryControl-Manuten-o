...
export function MaintenanceReports({
  ...
  const byEquipment = (machineId?: string) =>
    selectedEquipment === "all" || machineId === selectedEquipment;
  const filteredRequests = () =>
    requests.filter(
      (r) =>
        byEquipment(r.machineId) &&
        withinDate(
          r.completedAt || r.startedAt || r.assignedAt || r.requestedAt,
        ),
    );
  const filteredPlans = () =>
    (plans || []).filter(
      (p: any) =>
        byEquipment(p.machineId) &&
        withinDate(p.completedDate || p.scheduledDate),
    );
  const filteredDowntimes = () =>
    downtimes.filter(
      (d) => byEquipment(d.machineId) && withinDate(d.endTime || d.startTime),
    );
  ...
  const generatePDFReport = () => {
    ...
    if (reportType === "summary") {
      ...
      const scheduledPlanned = plns.filter(
        (p: any) => p.status === "scheduled",
      ).length;
      const completedPlanned = plns.filter(
      ...
      reqs.forEach((r) => {
        ...
        addLine(title);
        const dateStr = new Date(r.completedAt || r.requestedAt).toLocaleString(
          "pt-PT",
        );
        ...
        const timeH =
          r.workHours != null
            ? `${r.workHours}h`
            : r.actualDowntime
              ? `${Math.round(r.actualDowntime / 60)}h`
              : "—";
        addLine(
          `Tempo: ${timeH} | Custo: €${(r.cost || 0).toFixed(2)} | Estado: ${r.status}`,
          5,
        );
        yPosition += 2;
      });
      ...
      plns.forEach((p: any) => {
        ...
        addLine(
          `Data: ${dateRef ? new Date(dateRef).toLocaleString("pt-PT") : "—"} | Técnico: ${p.technician || "—"}`,
          5,
        );
        ...
        const timeH =
          p.actualDuration != null
            ? `${p.actualDuration}h`
            : p.estimatedDuration != null
              ? `${p.estimatedDuration}h (estim.)`
              : "—";
        const costV =
          p.actualCost != null
            ? p.actualCost
            : p.estimatedCost != null
              ...
              : 0;
        addLine(
          `Tempo: ${timeH} | Custo: €${Number(costV).toFixed(2)} | Estado: ${p.status}`,
          5,
        );
        yPosition += 2;
      });
      ...
      const dts = filteredDowntimes();
      const completed = dts.filter(
        (d) => d.status === "completed" && typeof d.duration === "number",
      );
      ...
      const mttr = completed.length
        ? Math.round(totalDowntime / completed.length)
        : 0; // minutes
      ...
      const windowMinutes = Math.max(
        1,
        Math.round((now.getTime() - windowStart.getTime()) / 60000),
      );
      const availability = Math.max(
        0,
        Math.min(100, Math.round((1 - totalDowntime / windowMinutes) * 100)),
      );
      ...
      dts.forEach((d) => {
        ...
        if (!byMachine[key])
          byMachine[key] = { name: d.machineName, downtime: 0 };
        byMachine[key].downtime += d.duration || 0;
      });
      const topDowntime = Object.values(byMachine)
      ...
      topDowntime.forEach((m) =>
        lines.push(`• ${m.name || "—"}: ${Math.round(m.downtime)} min`),
      );
      ...
      const planActual = plns.reduce(
        (s: any, p: any) => s + (p.actualCost || 0),
        0,
      );
      const planEstimatedOnly = plns.reduce(
        (s: any, p: any) => s + (p.actualCost ? 0 : p.estimatedCost || 0),
        0,
      );
      const total = reqCost + planActual + planEstimatedOnly;
      const avgPerIntervention =
        reqs.length + plns.length ? total / (reqs.length + plns.length) : 0;

      const byMachineCosts: Record<string, { name: string; cost: number }> = {};
      reqs.forEach((r) => {
        const key = r.machineId;
        if (!byMachineCosts[key])
          byMachineCosts[key] = { name: r.machineName, cost: 0 };
        byMachineCosts[key].cost += r.cost || 0;
      });
      plns.forEach((p: any) => {
        const key = p.machineId;
        if (!byMachineCosts[key])
          byMachineCosts[key] = { name: p.machineName, cost: 0 };
        byMachineCosts[key].cost += p.actualCost || p.estimatedCost || 0;
      });
      const topCosts = Object.values(byMachineCosts)
      ...
      lines.push(
        `Custo Médio por Intervenção: €${avgPerIntervention.toFixed(2)}`,
      );
      lines.push(`Solicitações (corretivas): €${reqCost.toFixed(2)}`);
      lines.push(
        `Planos (atuais): €${planActual.toFixed(2)} | Planos (estimados): €${planEstimatedOnly.toFixed(2)}`,
      );
      lines.push("");
      lines.push("Equipamentos com Maior Custo:");
      topCosts.forEach((m) =>
        lines.push(`• ${m.name || "—"}: €${m.cost.toFixed(2)}`),
      );
      ...
      const plns = filteredPlans().sort(
        (a: any, b: any) =>
          new Date(b.scheduledDate || b.completedDate).getTime() -
          new Date(a.scheduledDate || a.completedDate).getTime(),
      );
      const totalCount = reqs.length + plns.length;
      doc.text(`Total de intervenções: ${totalCount}`, 25, yPosition);
      yPosition += 8;
      ...
      doc.text(
        "NOTA: Este é um modelo simplificado. Para checklist completo",
        25,
        yPosition,
      );
      doc.text(
        "utilize a funcionalidade específica de Checklist DL50.",
        25,
        yPosition + 8,
      );
      yPosition += 25;
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      "Gerado pelo FactoryControl - Sistema de Gestão de Chão de Fábrica",
      20,
      pageHeight - 10,
    );
