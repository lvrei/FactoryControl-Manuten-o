...
export function ROIEditor({ cameraId, snapshotUrl, rois, onChange }: ROIEditorProps) {
    ...
    <div className="space-y-4">
      ...
      {rois.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-muted-foreground">Zonas Configuradas ({rois.length})</h4>
          <div className="grid gap-2">
            {rois.map((roi, index) => {
              const typeInfo = ANALYSIS_TYPES.find(t => t.value === roi.analysisType);
              const color = ROI_COLORS[index % ROI_COLORS.length];
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground mt-1">
                          Posição: {roi.coordinates.x.toFixed(1)}%, {roi.coordinates.y.toFixed(1)}% • 
                          Tamanho: {roi.coordinates.width.toFixed(1)}% × {roi.coordinates.height.toFixed(1)}%
                        </div>
                      </div>
                    </div>
          ...
          <div className="bg-background rounded-2xl max-w-lg w-full shadow-2xl border border-border/40">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold">Configurar Zona de Interesse</h3>
                <button
                  onClick={resetForm}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setForm({ ...form, analysisType: e.target.value as ROI['analysisType'] })}
                ...
                <div className="bg-muted/30 rounded-xl p-3">
                  <h4 className="text-sm font-semibold mb-2">Coordenadas da Zona</h4>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <span className="text-muted-foreground">X:</span>{' '}
                      <span className="font-mono">{editingRoi.coordinates.x.toFixed(1)}%</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Y:</span>{' '}
                      <span className="font-mono">{editingRoi.coordinates.y.toFixed(1)}%</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Largura:</span>{' '}
                      <span className="font-mono">{editingRoi.coordinates.width.toFixed(1)}%</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Altura:</span>{' '}
                      <span className="font-mono">{editingRoi.coordinates.height.toFixed(1)}%</span>
                    </div>
                  </div>
                </div>
