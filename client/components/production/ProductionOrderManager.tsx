...
export function ProductionOrderManager({
  onClose,
  editingOrder,
  onOrderCreated,
  initialLines,
  ...
  const loadData = async () => {
    try {
      ...
      const safeFoams = Array.isArray(foams)
        ? foams.filter(
            (foam) => foam && typeof foam === "object" && foam.id && foam.name,
          )
        : [];

      const safeMachines = Array.isArray(machinesData)
        ? machinesData.filter(
            (machine) =>
              machine &&
  ...
  const updateLine = (
    lineId: string,
    updates: Partial<ProductionOrderLine>,
  ) => {
    setLines((prev) =>
  ...
  const updateOperation = (
    lineId: string,
    operationId: string,
    updates: Partial<CuttingOperation>,
  ) => {
    const line = lines.find((l) => l.id === lineId);
    if (!line) return;
  ...
  const removeOperation = (lineId: string, operationId: string) => {
    ...
    const updatedOperations = line.cuttingOperations.filter(
  ...
  const computeWasteForLine = (line: ProductionOrderLine): WasteInfo[] => {
    ...
    (line.cuttingOperations || []).forEach((op) => {
      ...
      if ((out.length || 0) > (bzmDims.length || 0))
        unitAlerts.push("Comprimento superior ao das Medidas Finais");
      if ((out.width || 0) > (bzmDims.width || 0))
        unitAlerts.push("Largura superior à das Medidas Finais");
    ...
    const aggWasteLength =
      totalLengthAvailable > 0
        ? ((totalLengthAvailable - usedLenCapped) / totalLengthAvailable) * 100
        : 0;
    const aggWasteWidth =
      totalWidthAvailable > 0
        ? ((totalWidthAvailable - usedWidCapped) / totalWidthAvailable) * 100
        : 0;
    const aggWasteHeight =
      totalHeightAvailable > 0
        ? ((totalHeightAvailable - usedHeiCapped) / totalHeightAvailable) * 100
        : 0;
    ...
    if (aggHeiUsed > totalHeightAvailable) {
      aggAlerts.push(
        `Quantidade × espessura excede a disponível (${aggHeiUsed}mm > ${totalHeightAvailable}mm)`,
      );
    }

    // Se existir pelo menos uma operação downstream, retornar apenas o total da linha
    if (
      (line.cuttingOperations || []).some((op) =>
        ["CAROUSEL", "PRE_CNC", "CNC"].includes(getMachineType(op.machineId)),
      )
    ) {
      infos.push({
        opId: "TOTAL_LINHA",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      alert(
        "Preencha todos os campos obrigatórios e adicione pelo menos uma linha",
      );
      return;
    }

    // Validar se todas as linhas têm pelo menos uma operação
    const linesWithoutOperations = lines.filter(
      (line) => line.cuttingOperations.length === 0,
    );
                    ...
                    >
                      ...
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-4">
                        <div>
                          ...
                          >
                            {(foamTypes || [])
                              .filter((foam) => foam && foam.id && foam.name)
                              .map((foam) => (
                                ...
                                const updatedOperations =
                                  line.cuttingOperations.map((op) => {
                                    ...
                                    if (machine.type === "BZM") {
                                      return {
                                        ...op,
                                        inputDimensions: newFinalDimensions,
                                        outputDimensions: newFinalDimensions,
                                      };
                                    }
                                    if (
                                      machine.type === "PRE_CNC" ||
                                      machine.type === "CNC"
                                    ) {
                                      return {
                                        ...op,
                      ...
                      {line.cuttingOperations.length > 0 && (
                        <div>
                          <h5 className="font-medium mb-3">
                            Operações de Corte ({line.cuttingOperations.length})
                          </h5>
                          <div className="space-y-3">
                            {line.cuttingOperations.map(
                              (operation, opIndex) => (
                                <div
                                  key={operation.id}
                                ...
                                >
                                  <div className="flex items-center justify-between mb-3">
                                    ...
                                    <button
                                      type="button"
                                      onClick={() =>
                                        removeOperation(line.id, operation.id)
                                      }
                                      ...
                                      <div>
                                        ...
                                        <select
                                          ...
                                          onChange={(e) => {
                                            ...
                                            const updates: Partial<CuttingOperation> =
                                            ...
                                            if (
                                              ...
                                              updates.inputDimensions =
                                                line.finalDimensions;
                                            }
                                            // Se for BZM, a saída também acompanha as dimensões finais da linha
                                            if (
                                              selectedMachine &&
                                              selectedMachine.type === "BZM"
                                            ) {
                                              updates.outputDimensions =
                                                line.finalDimensions;
                                            }
                      ...
                      {line.cuttingOperations.length === 0 && (
                        <div className="text-center py-4 text-muted-foreground border border-dashed rounded">
                          ...
                          <p className="text-sm">
                            Nenhuma operação de corte definida
                          </p>
                          <p className="text-xs">
                            Adicione pelo menos uma operação para esta linha
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-5">
                  ...
                  <div>
                    ...
                    <div className="text-xl font-bold text-blue-600">
                      {calculateTotalLength().toFixed(2)} m
                    </div>
                    <div className="text-xs text-muted-foreground">
                      Dimensão final × blocos
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-muted-foreground">
            ...
            {lines.length > 0 && (
              <div className="border rounded-lg p-6 bg-muted/20">
                <h3 className="text-lg font-semibold mb-4">
                  Aproveitamento por Linha (BZM vs Operações Seguintes)
                </h3>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Blocos BZM: {line.quantity}
                          </div>
                        </div>
                        {!hasDownstream ? (
                          <div className="text-sm text-muted-foreground">
                            Apenas BZM — sem desperdício adicional calculado.
                          </div>
                        ) : (
                            ...
                            {infos.map((info) => (
                              <div key={info.opId} className="text-sm">
                                <div className="flex items-center justify-between">
                                  <div className="font-medium">
                                    {info.machineName} ({info.machineType})
                                  </div>
                                  <div className="text-xs text-muted-foreground">
                                    Qtd OP:{" "}
                                    {(line.cuttingOperations || []).find(
                                      (o) => o.id === info.opId,
                                    )?.quantity || 0}
                                  </div>
                                </div>
                                <div className="grid gap-2 md:grid-cols-4 mt-1">
                                  <div>
                                    <div className="text-xs text-muted-foreground">
                                      Desperdício Comprimento (total)
                                    </div>
                                    <div className="font-medium">
                                      {info.wasteLengthPctTotal.toFixed(1)}%
                                    </div>
                                  </div>
                                  <div>
                                    <div className="text-xs text-muted-foreground">
                                      Desperdício Largura (total)
                                    </div>
                                    <div className="font-medium">
                                      {info.wasteWidthPctTotal.toFixed(1)}%
                                    </div>
                                  </div>
                                  <div>
                                    <div className="text-xs text-muted-foreground">
                                      Desperdício Espessura (total)
                                    </div>
                                    <div className="font-medium">
                                      {info.wasteHeightPctTotal.toFixed(1)}%
                                    </div>
                                  </div>
                                  <div>
                                    <div className="text-xs text-muted-foreground">
                                    ...
                                    <div className="font-medium">
                                      {info.totalHeightUsed}mm de{" "}
                                      {info.totalHeightAvailable}mm
                                    </div>
                                  </div>
                                </div>
