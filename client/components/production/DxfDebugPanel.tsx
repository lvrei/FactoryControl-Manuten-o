    ...
    <div className="border rounded bg-muted/10">
      ...
      >
        <div className="flex items-center gap-2 text-sm">
          ...
          {hasIssues && <AlertCircle className="h-4 w-4 text-yellow-600" />}
        </div>
        {isExpanded ? (
      ...
      {isExpanded && (
        <div className="p-3 border-t text-xs space-y-3">
              ...
              <div className="ml-6 space-y-1 max-h-40 overflow-auto">
                {drawing.parts.map((part, idx) => (
                  <div key={idx} className="font-mono text-muted-foreground">
                    #{idx + 1}: {Math.round(part.length)}×
                    {Math.round(part.width)}×{Math.round(part.height)}mm (qty:{" "}
                    {part.quantity}){part.label && ` - ${part.label}`}
                  </div>
                ))}
              </div>
            )}
          ...
          {drawing.bbox && (
            <div className="p-2 bg-background rounded">
              ...
              <div className="font-mono text-muted-foreground space-y-1">
                <div>
                  X: [{drawing.bbox.minX.toFixed(2)},{" "}
                  {drawing.bbox.maxX.toFixed(2)}]
                </div>
                <div>
                  Y: [{drawing.bbox.minY.toFixed(2)},{" "}
                  {drawing.bbox.maxY.toFixed(2)}]
                </div>
                <div>
                  Tamanho: {(drawing.bbox.maxX - drawing.bbox.minX).toFixed(2)}{" "}
                  × {(drawing.bbox.maxY - drawing.bbox.minY).toFixed(2)}
                </div>
              </div>
            </div>
          )}
          ...
          {hasIssues && (
            <div className="p-2 bg-yellow-50 border border-yellow-200 rounded text-yellow-800">
              ...
              {drawing.parts.length === 0 && drawing.paths.length === 0 && (
                <span>
                  Nenhuma geometria foi detetada. Verifique se o DXF contém
                  entidades válidas.
                </span>
              )}
              {drawing.parts.length === 0 && drawing.paths.length > 0 && (
                <span>
                  Geometria visualizada mas sem retângulos detetados. Use
                  seleção manual ou forneça JSON.
                </span>
              )}
            </div>
          )}
          ...
          <div className="pt-2 border-t text-muted-foreground">
            ...
            <div>
              Abra a consola do navegador (F12) para ver logs detalhados do
              processamento DXF.
            </div>
          </div>
        </div>
