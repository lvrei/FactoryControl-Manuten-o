...
import { NestPart, Sheet, packRectangles, parseDxfRectangles, parseJsonParts } from "@/lib/nesting";
...
export default function NestingModal({ onClose, onApply }: NestingModalProps) {
  ...
  const [sheet, setSheet] = useState<Sheet>({ length: 2000, width: 1000, kerf: 5, margin: 10 });
  ...
  useEffect(() => {
    if (!result) { setStockWarning(""); return; }
    // Rough stock check: compare total volume to available blocks of selected foamType
    const totalVolumeM3 = parts.reduce((s, p) => s + (p.length * p.width * p.height * p.quantity) / 1e9, 0);
    if (stockBlocks && mappingFoamTypeId) {
      const availableBlocks = stockBlocks.filter(b => b.foamType?.id === mappingFoamTypeId && b.status === "available");
      const availableVol = availableBlocks.reduce((s, b) => s + (b.volume || 0), 0);
      if (availableVol + 1e-9 < totalVolumeM3) {
        setStockWarning(`Aviso: stock insuficiente para ${totalVolumeM3.toFixed(3)} m³; disponível ${availableVol.toFixed(3)} m³.`);
      } else {
        setStockWarning("");
      }
    } else {
  ...
  function applyToOrder() {
    ...
    for (const p of parts) {
      ...
      if (!map.has(key)) map.set(key, { part: { ...p, foamTypeId: foamId }, qty: 0 });
      const rec = map.get(key)!;
      rec.qty += p.quantity;
    }
    const lines: ProductionOrderLine[] = [] as any;
    for (const { part, qty } of map.values()) {
      const foam = foamTypes.find(f => f.id === (part.foamTypeId || mappingFoamTypeId)) || foamTypes[0];
      if (!foam) continue;
      lines.push({
        id: `line-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,
        foamType: foam,
        initialDimensions: { length: sheet.length, width: sheet.width, height: part.height },
        finalDimensions: { length: part.length, width: part.width, height: part.height },
        quantity: qty,
        completedQuantity: 0,
        cuttingOperations: [],
        status: "pending",
      ...
      <div className="bg-background rounded-lg w-full max-w-5xl max-h-[90vh] overflow-auto">
        <div className="p-4 border-b flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Importe desenhos e gere linhas para OP. Visualização do 1º painel.</p>
          </div>
        ...
        <div className="p-4 grid md:grid-cols-3 gap-4">
          <div className="md:col-span-1 space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Mapear Tipo de Espuma</label>
              <select value={mappingFoamTypeId} onChange={(e)=>setMappingFoamTypeId(e.target.value)} className="w-full border rounded px-2 py-1">
                <option value="">Selecionar (ou use foamTypeId no JSON)</option>
                {foamTypes.map(ft=> <option key={ft.id} value={ft.id}>{ft.name}</option>)}
              </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="number" value={sheet.width} onChange={e=>setSheet(s=>({...s,width:Number(e.target.value)}))} className="w-full border rounded px-2 py-1" />
              </div>
              <div>
                <label className="block text-sm">Comprimento painel (mm)</label>
                <input type="number" value={sheet.length} onChange={e=>setSheet(s=>({...s,length:Number(e.target.value)}))} className="w-full border rounded px-2 py-1" />
              </div>
              <div>
                <label className="block text-sm">Kerf (mm)</label>
                <input type="number" value={sheet.kerf} onChange={e=>setSheet(s=>({...s,kerf:Number(e.target.value)}))} className="w-full border rounded px-2 py-1" />
              </div>
              <div>
                <label className="block text-sm">Margem (mm)</label>
                <input type="number" value={sheet.margin} onChange={e=>setSheet(s=>({...s,margin:Number(e.target.value)}))} className="w-full border rounded px-2 py-1" />
              </div>
            </div>

            {result && (
              <div className="p-2 border rounded bg-muted/30 text-sm">
                <div className="flex items-center gap-2"><Package className="h-4 w-4"/> Painéis necessários: <strong>{result.sheetsUsed}</strong></div>
                <div>Utilização: {(result.utilization*100).toFixed(1)}%</div>
              </div>
            )}

            {stockWarning && (
              <div className="p-2 border rounded bg-yellow-50 text-yellow-700 text-sm flex gap-2"><AlertTriangle className="h-4 w-4"/>{stockWarning}</div>
            )}

            <div className="flex gap-2 pt-2">
              <button onClick={onClose} className="px-3 py-2 border rounded">Cancelar</button>
              <button onClick={applyToOrder} className="px-3 py-2 bg-primary text-primary-foreground rounded disabled:opacity-50" disabled={!result || parts.length===0}>
                Aplicar na OP
              </button>
            </div>
          </div>
          ...
          <div className="md:col-span-2 border rounded p-2 bg-white overflow-auto">
            ...
            <svg width={Math.max(420, sheet.width*svgScale)} height={(sheet.length*svgScale)+20} style={{background:'#f7fafc'}}>
              {/* panel outline */}
              <rect x={10} y={10} width={sheet.width*svgScale} height={sheet.length*svgScale} fill="#fff" stroke="#e2e8f0" />
              {firstSheetPlacements.map((p, idx)=>{
                ...
                return (
                  <g key={idx}>
                    ...
                    <text x={x+4} y={y+12} fontSize={10} fill="#1a202c">{Math.round(p.length)}×{Math.round(p.width)}×{Math.round(p.height)}</text>
                  </g>
                );
              })}
            </svg>
