...
export default function MachinePage() {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'maintenance' | 'sensors' | 'cameras' | 'chat' | 'qr'>('info');
  ...
  const loadMachineData = async () => {
    try {
      ...
      if (foundMachine) {
        ...
        setMaintenanceRequests(requests.filter(r => r.machineId === machineId));
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-lg text-muted-foreground">Carregando equipamento...</div>
      </div>
    );
  }
      ...
      <div className="sticky top-0 z-40 border-b border-border/40 bg-gradient-to-r from-card/95 via-card/90 to-card/95 backdrop-blur-xl shadow-lg">
        <div className="p-4">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={`px-4 py-2 rounded-full text-sm font-bold ${getStatusColor(machine.status)}`}>
      ...
      <div className="sticky top-[105px] z-30 border-b border-border/40 bg-card/95 backdrop-blur-sm">
        <div className="flex overflow-x-auto scrollbar-hide px-4">
          {[
            ...
            { id: 'maintenance', label: 'Manutenção', icon: Wrench, badge: maintenanceRequests.length },
            { id: 'sensors', label: 'Sensores', icon: Activity },
            { id: 'cameras', label: 'Câmaras', icon: Camera, badge: cameras.length },
      ...
      <div className="p-4 max-w-6xl mx-auto">
            ...
            <div className="rounded-2xl border border-border/40 bg-gradient-to-br from-card via-card to-card/95 p-6 shadow-lg">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="font-semibold font-mono">SN-2023-{machine.id}</p>
                </div>
                <div>
              ...
              <div className="rounded-2xl border border-border/40 bg-gradient-to-br from-card via-card to-card/95 p-6 shadow-lg">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-primary">{machine.maxDimensions.length}mm</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Largura</p>
                    <p className="text-2xl font-bold text-primary">{machine.maxDimensions.width}mm</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Altura</p>
                    <p className="text-2xl font-bold text-primary">{machine.maxDimensions.height}mm</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            {maintenanceRequests.length === 0 ? (
              <div className="rounded-2xl border border-border/40 bg-gradient-to-br from-card via-card to-card/95 p-12 text-center">
                ...
                <h3 className="text-xl font-bold mb-2">Sem Pedidos de Manutenção</h3>
                <p className="text-muted-foreground">Nenhum pedido registado para este equipamento</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-center gap-2 mt-2 text-xs">
                        <span className={`px-2 py-1 rounded-full font-semibold ${
                          ...
                          {request.urgencyLevel === 'critical' ? 'Crítico' :
                           request.urgencyLevel === 'high' ? 'Alta' : 'Média'}
                        </span>
        ...
        {activeTab === 'sensors' && (
          <div className="rounded-2xl border border-border/40 bg-gradient-to-br from-card via-card to-card/95 p-12 text-center">
            ...
            <p className="text-muted-foreground">Informação de sensores em desenvolvimento</p>
          </div>
        )}
          ...
          <div className="space-y-4">
            {cameras.length === 0 ? (
              <div className="rounded-2xl border border-border/40 bg-gradient-to-br from-card via-card to-card/95 p-12 text-center">
                ...
                <h3 className="text-xl font-bold mb-2">Sem Câmaras Associadas</h3>
                <p className="text-muted-foreground">Configure câmaras para este equipamento</p>
                <button
