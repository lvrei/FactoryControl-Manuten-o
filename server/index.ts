...
export async function createServer() {
  ...
  // Equipment Maintenance Schedules endpoints (defined BEFORE routers to avoid conflicts)
  ...
  // GET endpoint for equipment maintenance schedules
  app.get(["/api/equipment-schedules", "/equipment-schedules"], async (req, res) => {
    try {
      if (!isDbConfigured()) return res.json([]);
  ...
  // POST endpoint for creating maintenance schedules
  app.post(["/api/equipment-schedules", "/equipment-schedules"], async (req, res) => {
    try {
      if (!isDbConfigured())
        return res.status(400).json({ error: "Database not configured" });
  ...
  // PUT endpoint for updating maintenance schedules
  app.put(["/api/equipment-schedules/:id", "/equipment-schedules/:id"], async (req, res) => {
    try {
      if (!isDbConfigured())
        return res.status(400).json({ error: "Database not configured" });
  ...
  // DELETE endpoint for maintenance schedules
  app.delete(["/api/equipment-schedules/:id", "/equipment-schedules/:id"], async (req, res) => {
    try {
      if (!isDbConfigured())
        return res.status(400).json({ error: "Database not configured" });
  ...
  // POST endpoint to reschedule maintenance (called when a maintenance is completed)
  app.post(["/api/equipment-schedules/:id/reschedule", "/equipment-schedules/:id/reschedule"], async (req, res) => {
    try {
      if (!isDbConfigured())
        return res.status(400).json({ error: "Database not configured" });
